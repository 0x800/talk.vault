	...
	if(curTab.hasClass("tab-conv")){
		...
		if(header){
			if(listItem.id.indexOf("-") != -1){
				...
				convHeader.find("#convDisplayName").text(listItem.username);
				convHeader.find("#convSubtitleStatusMessage").text(listItem.displayname);
...
function createItemHTML(type, item){
	if(type == "conv"){
		...
		if(item.id.indexOf("-") != -1){
			...
			return '<div class="sidebarListItem' + (currentTab==item.id ? ' activeItem' : '') + '" data-item="' + escapeText(item.id) + '" data-trigger="conv" data-status="' + (typeof item.status != 'undefined' ? item.status : "") + '">' + pinnedIcon + '<div class="unreadBadge">0</div><img src="' + (item.avatar ? escapeText(item.avatar) : 'img/noavatar.png') + '" class="listItemIcon" /><span class="listItemSubtitle">' + escapeText(subtitle)  + '</span><br /><span class="listItemTitle">' + escapeText(item.displayname || item.username) + '</span></div>';
		} else {
