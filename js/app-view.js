...
function hooks(){
	var network = "wss://talk.vault.fr/server/";
	...
	if(document.location.hash == "#local")
		network = "wss://" + document.location.hostname + "/server/";
...
function loggedInCalls(){
	...
	if(document.location.hostname != "talk.vault.fr"){
...
function newNotification(icon, title, content, closeAfter, showWithFocus){
	...
	if(!hasFocus && !closeAfter){
		titleBadgeCount++;
		document.title = "(" + titleBadgeCount + ") Skipe";
